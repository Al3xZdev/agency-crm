...
describe('reviews/decision API (slice 7)', () => {
  ...
  it('casts APPROVED (201)', async () => {
    const auth = staffAuth('ACCOUNT_MANAGER');
    const res = await request(app.getHttpServer())
      .post('/versions/ver_1/decision')
      .set('Cookie', auth.cookie)
      .set(auth.headers)
      .send({ decision: 'APPROVED' });
    expect(res.status).toBe(201);
    expect(res.body).toMatchObject({ decision: 'APPROVED', actorType: 'STAFF' });
    expect(db._creatives.get('cr_1')?.status).toBe('APPROVED');
  });

  it('casts REJECTED (201)', async () => {
    const auth = staffAuth('ACCOUNT_MANAGER');
    const res = await request(app.getHttpServer())
      .post('/versions/ver_1/decision')
      .set('Cookie', auth.cookie)
    ...
    expect(res.status).toBe(201);
    expect(res.body).toMatchObject({ decision: 'REJECTED' });
    expect(db._creatives.get('cr_1')?.status).toBe('REJECTED');
  });

  it('casts REQUEST_CHANGES (201)', async () => {
    const auth = staffAuth('ACCOUNT_MANAGER');
    const res = await request(app.getHttpServer())
      .post('/versions/ver_1/decision')
      .set('Cookie', auth.cookie)
    ...
    expect(res.status).toBe(201);
    expect(res.body).toMatchObject({ decision: 'REQUEST_CHANGES' });
    expect(db._creatives.get('cr_1')?.status).toBe('CHANGES_REQUESTED');
  });
  ...
  it('returns 404 when version does not exist', async () => {
    const auth = staffAuth('ACCOUNT_MANAGER');
    const res = await request(app.getHttpServer())
      .post('/versions/nonexistent/decision')
      .set('Cookie', auth.cookie)
      .set(auth.headers)
      .send({ decision: 'APPROVED' });
