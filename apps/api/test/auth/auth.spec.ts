...
function buildMockDb() {
  ...
  const db = {
    ...
    loginAttempt: {
      ...
      deleteMany: ({ where }: { where: { emailLower: string } }) => {
        const before = loginAttempts.length;
        loginAttempts = loginAttempts.filter((a) => a.emailLower !== where.emailLower);
...
describe('staff auth (slice 2)', () => {
  ...
  async function authed(email: string, password: string, method: 'get' | 'post', url: string) {
    ...
    const csrfCookie = cookieOf(res, '__Host-csrf');
  ...
  it('sets secure cookie flags on login: session HttpOnly+Lax, csrf readable', async () => {
    ...
    const csrf = cookieOf(res, '__Host-csrf')!;
    expect(session).toContain('HttpOnly');
    expect(session.toLowerCase()).toContain('samesite=lax');
    expect(session).toContain('Path=/');
    // dev/test over plain http: Secure off; production forces it (unit-checked separately)
