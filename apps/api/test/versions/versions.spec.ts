...
describe('versions read API (slice 7)', () => {
  ...
  beforeEach(async () => {
    ...
    db._seedVersion('ver_2', 'cr_1', 'client_1', { versionNo: 2, state: 'PROCESSING' });
    db._seedVersion('ver_3', 'cr_1', 'client_1', { versionNo: 3, state: 'READY', reviewStatus: 'APPROVED' });
  ...
  it('GET /creatives/:creativeId/versions returns versions ordered by versionNo DESC', async () => {
    const auth = staffAuth('ACCOUNT_MANAGER');
    const res = await request(app.getHttpServer())
      .get('/creatives/cr_1/versions')
  ...
  it('GET /creatives/:creativeId/versions returns empty array when no versions', async () => {
    const auth = staffAuth('ACCOUNT_MANAGER');
    db._seedCreative('cr_empty', 'cmp_1', 'client_1', 'TEXT');
    const res = await request(app.getHttpServer())
      .get('/creatives/cr_empty/versions')
      .set('Cookie', auth.cookie);
    expect(res.status).toBe(200);
    expect(res.body).toEqual([]);
  });

  it('GET /versions/:versionId returns version detail with asset info', async () => {
    const auth = staffAuth('ACCOUNT_MANAGER');
    const res = await request(app.getHttpServer())
      .get('/versions/ver_3')
      .set('Cookie', auth.cookie);
    ...
    expect(res.body).toHaveProperty('posterUrl');
  });

  it('GET /versions/:versionId returns 404 when version does not exist', async () => {
    const auth = staffAuth('ACCOUNT_MANAGER');
    const res = await request(app.getHttpServer())
      .get('/versions/nonexistent')
      .set('Cookie', auth.cookie);
